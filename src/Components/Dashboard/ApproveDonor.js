...
const ApproveDonor = () => {
    ...
    const handleDeny = (id) => {
        axios.delete(`https://pear-gifted-lamb.cyclic.app/admin/removeDonorReq?id=${id}`, {
            headers: {
                'Content-Type': "application/json",
                Authorization: `Bearer ${token}`
            }
        ...
        <div>
            ...
            <div className="overflow-x-auto px-[10px]">
                <table className="table w-full">
                    ...
                    <tbody>
                        {
                            donorRequest.map((donorRq, index) => <tr className="border-[#222]">
                                <th>{index + 1}</th>
                                ...
                                <td>
                                    <button onClick={() => handleApprove(donorRq?._id)} className="btn no-animation gap-2 bg-green-500 mr-[14px] hover:bg-primary hover:border-primary text-white">
                                        Approve
                                    </button>
                                </td>
                                <td>
                                    <button onClick={() => handleDeny(donorRq?._id)} className="btn no-animation gap-2 bg-primary hover:bg-primary hover:border-primary  text-white">
                                        Deny
                                    </button>
                                </td>
                                ...
                                <div className="modal">
                                    <div className="modal-box relative">
                                        ...
                                        <h3 className="text-lg font-bold">Health data for {donorRq.user.email.split("@")[0]}</h3>
                                        <ul>
                                            <li>HIV: {donorRq.hiv}</li>
                                            <li>Date of Birth: {donorRq.dateOfBirth.split("T")[0]}</li>
                                            <li>Hypertension: {donorRq.hypertension}</li>
                                            <li>Last Blood donation: {donorRq.lastBloodDonation}</li>
